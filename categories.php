<?php 
     ...
     if(isset($_REQUEST['cat'])) {
            $cat =  $_REQUEST['cat'];
            $catsql =  "SELECT * FROM categories WHERE cId = '$cat'";
            $cats = mysqli_query($conn, $catsql);
            $catrow = mysqli_fetch_assoc($cats);
            $category = $catrow['cName'];
        } 
      else {
        $cat = 0;
    }   
...
$gal=mysqli_query($conn,"SELECT * FROM `product_info` WHERE cId ='$cat';");
